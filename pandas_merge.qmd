...
```{python}
# primary data set `student`
pd.merge(students, exam_dates, how="left")
```

```{python}
# reverse
pd.merge(exam_dates,students, how="left")
```
...
Which dataframe should be the left one? 
**tb_cases_children**
And which should be the right one?
**country_continents**
...
```{python}
# this join is less intuitive and data analysts do not typically use right join
pd.merge(people, test_info_xavier, how="right")
```

```{python}
# left join is more intuitive and data analysts opt left join
pd.merge(test_info_xavier, people, how="left")
```

## inner join

Only keep the rows that are matched.
...
This join preserves all the records from both/all data sets
