...
```{python}
loan = pd.read_csv("data/loans_full_schema.csv")
loan
```
...
```{python}
loan_dropped = loan.drop(columns=["rownames"])
loan_dropped
```
...
```{python}
demog_loan_info = loan_dropped[["emp_title",
"emp_length",
"state",
"homeownership",
"loan_amount",
...
```{python}
credit_vars = loan_dropped.filter(regex = "credit")
...
```{python}
high_income = loan_dropped.query("annual_income > 100000")
...
```{python}
high_risk = loan_dropped.query("interest_rate > 15 | debt_to_income > 30 | delinq_2y > 0")
...
```{python}
prime = loan_dropped.query("grade.str.contains('A|B') & debt_to_income < 20 & delinq_2y == 0")
