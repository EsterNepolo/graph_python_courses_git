...
```{python}
fig = px.histogram(pa, x="total_pa_m_wk", nbins=5, title="Distribution of Total Physical Activity<br>per Week")
fig.update_layout(
    xaxis_title="Total Physical Activity (min/week)"  
)
fig.show()
```
...
fig = px.box(
    ...
    color="status",
    labels={
        "total_pa_m_wk": "Total Physical Activity (minutes/week)",
        "status": "Age Group"
    },
    title="Distribution of Weekly Physical Activity <br> by Age Group"
)
...
The plot compares weekly physical activity levels between Adults and Children. Each box represents the distribution of total minutes spent on physical activity per week for each age group.
Overall, this plot highlights that adults generally engage in more physical activity per week than children, with more variability in their activity levels.
